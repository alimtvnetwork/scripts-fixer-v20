...
This guide provides a comprehensive, step-by-step procedure for managing and configuring an Ubuntu system from scratch.
...
```bash
sudo apt-fast install -y build-essential libssl-dev
sudo apt-fast install -y curl wget git vim nano htop gedit
sudo apt-fast install -y aria2 # For parallel downloads
sudo apt-fast install -y bleachbit
```

## 3. ZSH & Oh-My-Zsh Configuration
Ubuntu's default shell is bash, but we upgrade to ZSH for improved productivity.
- Install ZSH:
  ```bash
...
- **DBeaver**:
  ...
  # Or via deb: sudo dpkg -i dbeaver-ce_latest_amd64.deb
  ```
- **GitHub Desktop**:
  ```bash
