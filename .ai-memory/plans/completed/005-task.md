...
citations:
  ...
  database: "n/a - no db"
  ui_surface: "n/a - cli"
  tests: "unit test_route_phase_5()"
  ci_cd_guard: "linter-scripts/check-sh-syntax.sh"
  ambiguity: "n/a"
  issue_rca: "n/a"
---
# Task 005 — Implement CLI router core phase 5

## 1. Learn
- [Bash guide](02-spec/02-coding-guidelines/04-bash/00-overview.md) - Context for route_phase_5()
- [CLI architecture](02-spec/21-app/07-error-and-logging/01-error-code-allocation.md) - Error codes
- [Strict avoid](.ai-memory/strictly-avoid.md) - Prevent regression 5

## 2. Goal
Build the argument parser and routing logic for the run command phase 5. This affects the `scripts/run.sh` file and enables the execution of route_phase_5(). Blast radius is contained to this specific installation phase.

## 3. Inputs and Contracts
Input: CLI flags for route_phase_5().
Output: Exit code 0 on success.

## 4. Execute
1. Open `scripts/run.sh`.
2. Implement `route_phase_5()` handling the specific logic for Implement CLI router core phase 5.

## 5. Constraints
- [Rule 1](02-spec/02-coding-guidelines/00-canonical-size-tier.md): Keep route_phase_5() under canonical size.
- [Rule 2](02-spec/03-error-manage/02-error-architecture/00-overview.md): Emit correct error codes.
- [Rule 3](.ai-memory/strictly-avoid.md): Do not mutate global state 5.
...
## 7. Done When
- [ ] 1. `route_phase_5()` is implemented.
