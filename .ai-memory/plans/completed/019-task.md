...
domain: Cli
phase: Implement
target_files: ["scripts/os/ubuntu/install-golang.sh"]
...
citations:
  ...
  database: "n/a - no db"
  ui_surface: "n/a - cli"
  tests: "unit test_install_golang()"
  ci_cd_guard: "linter-scripts/check-sh-syntax.sh"
  ambiguity: "n/a"
  issue_rca: "n/a"
---
# Task 019 — Implement golang installation script

## 1. Learn
- [Bash guide](02-spec/02-coding-guidelines/04-bash/00-overview.md) - Context for install_golang()
- [CLI architecture](02-spec/21-app/07-error-and-logging/01-error-code-allocation.md) - Error codes
- [Strict avoid](.ai-memory/strictly-avoid.md) - Prevent regression 19

## 2. Goal
Write the standalone installation script for golang ensuring idempotent execution. This affects the `scripts/os/ubuntu/install-golang.sh` file and enables the execution of install_golang(). Blast radius is contained to this specific installation phase.

## 3. Inputs and Contracts
Input: CLI flags for install_golang().
Output: Exit code 0 on success.

## 4. Execute
1. Open `scripts/os/ubuntu/install-golang.sh`.
2. Implement `install_golang()` handling the specific logic for Implement golang installation script.

## 5. Constraints
- [Rule 1](02-spec/02-coding-guidelines/00-canonical-size-tier.md): Keep install_golang() under canonical size.
- [Rule 2](02-spec/03-error-manage/02-error-architecture/00-overview.md): Emit correct error codes.
- [Rule 3](.ai-memory/strictly-avoid.md): Do not mutate global state 19.

## 6. Verify
```bash
bash -n scripts/os/ubuntu/install-golang.sh
```
Expected output: No syntax errors.

## 7. Done When
- [ ] 1. `install_golang()` is implemented.
