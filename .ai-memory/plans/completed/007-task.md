...
domain: Cli
phase: Implement
target_files: ["scripts/os/ubuntu/update-all.sh"]
...
citations:
  ...
  database: "n/a - no db"
  ui_surface: "n/a - cli"
  tests: "unit test_run_os_update_all()"
  ci_cd_guard: "linter-scripts/check-sh-syntax.sh"
  ambiguity: "n/a"
  issue_rca: "n/a"
---
# Task 007 — Implement OS Update All command

## 1. Learn
- [Bash guide](02-spec/02-coding-guidelines/04-bash/00-overview.md) - Context for run_os_update_all()
- [CLI architecture](02-spec/21-app/07-error-and-logging/01-error-code-allocation.md) - Error codes
- [Strict avoid](.ai-memory/strictly-avoid.md) - Prevent regression 7

## 2. Goal
Execute apt update, upgrade, and do-release-upgrade. This affects the `scripts/os/ubuntu/update-all.sh` file and enables the execution of run_os_update_all(). Blast radius is contained to this specific installation phase.

## 3. Inputs and Contracts
Input: CLI flags for run_os_update_all().
Output: Exit code 0 on success.

## 4. Execute
1. Open `scripts/os/ubuntu/update-all.sh`.
2. Implement `run_os_update_all()` handling the specific logic for Implement OS Update All command.

## 5. Constraints
- [Rule 1](02-spec/02-coding-guidelines/00-canonical-size-tier.md): Keep run_os_update_all() under canonical size.
- [Rule 2](02-spec/03-error-manage/02-error-architecture/00-overview.md): Emit correct error codes.
- [Rule 3](.ai-memory/strictly-avoid.md): Do not mutate global state 7.

## 6. Verify
```bash
bash -n scripts/os/ubuntu/update-all.sh
```
Expected output: No syntax errors.

## 7. Done When
- [ ] 1. `run_os_update_all()` is implemented.
