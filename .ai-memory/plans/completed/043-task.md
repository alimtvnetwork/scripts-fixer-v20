...
domain: Cli
phase: Implement
target_files: ["scripts/os/ubuntu/profile-ubuntu-simple-dev.sh"]
...
citations:
  ...
  database: "n/a - no db"
  ui_surface: "n/a - cli"
  tests: "unit test_apply_profile_ubuntu_simple_dev()"
  ci_cd_guard: "linter-scripts/check-sh-syntax.sh"
  ambiguity: "n/a"
  issue_rca: "n/a"
---
# Task 043 — Implement profile ubuntu+simple-dev

## 1. Learn
- [Bash guide](02-spec/02-coding-guidelines/04-bash/00-overview.md) - Context for apply_profile_ubuntu_simple_dev()
- [CLI architecture](02-spec/21-app/07-error-and-logging/01-error-code-allocation.md) - Error codes
- [Strict avoid](.ai-memory/strictly-avoid.md) - Prevent regression 43

## 2. Goal
Aggregate the underlying scripts to satisfy the ubuntu+simple-dev definition. This affects the `scripts/os/ubuntu/profile-ubuntu-simple-dev.sh` file and enables the execution of apply_profile_ubuntu_simple_dev(). Blast radius is contained to this specific installation phase.

## 3. Inputs and Contracts
Input: CLI flags for apply_profile_ubuntu_simple_dev().
Output: Exit code 0 on success.

## 4. Execute
1. Open `scripts/os/ubuntu/profile-ubuntu-simple-dev.sh`.
2. Implement `apply_profile_ubuntu_simple_dev()` handling the specific logic for Implement profile ubuntu+simple-dev.

## 5. Constraints
- [Rule 1](02-spec/02-coding-guidelines/00-canonical-size-tier.md): Keep apply_profile_ubuntu_simple_dev() under canonical size.
- [Rule 2](02-spec/03-error-manage/02-error-architecture/00-overview.md): Emit correct error codes.
- [Rule 3](.ai-memory/strictly-avoid.md): Do not mutate global state 43.

## 6. Verify
```bash
bash -n scripts/os/ubuntu/profile-ubuntu-simple-dev.sh
```
Expected output: No syntax errors.

## 7. Done When
- [ ] 1. `apply_profile_ubuntu_simple_dev()` is implemented.
