...
- **Notes:** Spec at `02-spec/install-bootstrap/readme.md`. Parallel HEAD probes via `Start-ThreadJob` (PS) / `xargs -P 20` (bash). Redirect-loop guard via `SCRIPTS_FIXER_REDIRECTED=1`.

### `scripts/models/` unified orchestrator
- **Implemented:** v0.32.0 (2026-04-17)
- **Notes:** Thin dispatcher delegates to `picker.ps1`. Env-var handoff contract: `LLAMA_CPP_INSTALL_IDS` / `OLLAMA_PULL_MODELS`. Documented in `02-spec/models/readme.md`.
...
See `.ai-memory/memory/suggestions/completed/` for detailed suggestion docs per script.
