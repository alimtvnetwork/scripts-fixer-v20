...
# Centralized CI Quality Gate Job Definitions
CI_JOBS_MATRIX: dict[str, list[str]] = {
    "Validate JSON Configs": ["node", "tools/validate-json-configs.mjs"],
    "Registry Sync Check": ["node", "tools/registry-sync.cjs", "--check"],
    "Sync Version Check": ["node", "tools/sync-version.cjs", "--check"],
    "Manifest Validate": ["node", "tools/manifest-validate.cjs"],
    "Docs Generate Check": ["node", "tools/docs-generate.cjs", "--check"],
    "Spec Lint": ["node", "tools/spec-lint.cjs"],
    "Allowlist Budget": ["node", "tools/allowlist-budget.cjs"],
    "Gen Completions Check": ["node", "tools/gen-completions.cjs", "--check"],
    "Manifest Aliases Check": ["node", "tools/manifest-aliases.cjs", "--check"],
    "Scan Legacy Fixer Refs": ["pwsh", "-File", "tools/scan-legacy-fixer-refs.ps1"],
    "PSScriptAnalyzer": ["pwsh", "-Command", "Invoke-ScriptAnalyzer -Path . -Recurse -Severity Error -ExcludeRule PSAvoidUsingWriteHost,PSUseShouldProcessForStateChangingFunctions,PSAvoidUsingConvertToSecureStringWithPlainText"],
    "Pester Test Suite": ["pwsh", "-Command", "$r = Invoke-Pester -Path tests/pester -PassThru; if ($r.FailedCount -gt 0) { exit 1 }"],
    ...
    "Boolean Naming Check": [sys.executable, "03-ai-scripts/08-naming-autofixer.py", "scripts"],
